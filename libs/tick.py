    ...
    if TICK == 0:
        if blockstates[tile_data["type"]]["on_ground"]:
            ...
            if info["bottom"] is not None and blockstates[info["bottom"]["type"]]["collision"] == False:
                if blockstates[tile_data["type"]]["on_himself"] == False or info["bottom"]["type"] != tile_data["type"]:
                    Entity.add(Item(tile[0]*16,tile[1]*16,tile_data["type"],tile_items[tile_data["type"]]))
